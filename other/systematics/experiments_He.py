...
print(f"s_α = {a2 - a1:.6f} deg, s_δ = {d2 - d1:.6f} deg")
...
# Kernel parameters
length_kernel_params = dict(initial=3.0, lower=1e-1, upper=1e1, fixed=True)
var_kernel_params = dict(initial=1.0, lower=1e-5, upper=1e3, fixed=True)
# shared_length_scale = ConstrainedParameter(**length_kernel_params)
A_length_scale = ConstrainedParameter(**length_kernel_params)
v_length_scale = ConstrainedParameter(**length_kernel_params)
...
A_kernel = Matern12(length_scale=A_length_scale, variance=A_variance)
v_kernel = Matern12(length_scale=v_length_scale, variance=v_variance)
# A_kernel = Matern12(length_scale=shared_length_scale, variance=A_variance)
# v_kernel = Matern12(length_scale=shared_length_scale, variance=v_variance)
...
plt.savefig("rosette_He_talk.pdf", bbox_inches="tight")
plt.show()
...
fig, ax = plt.subplots(3, 3, figsize=[9, 9], layout="compressed", sharex=True, sharey=True)

ax_flat = ax.flatten()

for i, j in enumerate(rng.choice(n_spaxels, 9, replace=False)):
