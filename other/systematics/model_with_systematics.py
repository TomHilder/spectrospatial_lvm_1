...
class EmissionLine(SpectralSpatialModel):
    ...
    σ_raw: SpatialModel  # Unconstrained line width
    ...
    def σ2_obs(self, s) -> Array:
        return self.σ_raw(s) ** 2 + self.σ_lsf(s) ** 2


class EmissionLineNew(SpectralSpatialModel):
    # Line centre in Angstroms
    μ: Parameter
    # Model components / line quantities
    A_raw: SpatialModel  # Unconstrained line flux
    ...
    def vσ(self, s) -> Array:
        # return l_bounded(self.vσ_raw(s), lower=V_LOWER)
...
class LVMModel(SpectralSpatialModel):
    ...
    def __init__(
        ...
        self.line = EmissionLine(
            ...
            σ_raw=FourierGP(n_modes=n_modes, kernel=σ_kernel),
...
class LVMModelNew(SpectralSpatialModel):
    # Model components
    line: EmissionLineNew  # Emission line model
    offs: PerSpaxel  # Nuisance offsets per spaxel
