...
λ_pad = 10
λ_range = (LINE_CENTRE - λ_pad, LINE_CENTRE + λ_pad)
...
shared_length_scale = ConstrainedParameter(initial=1.0, fixed=True, lower=0.01, upper=5)
σ_length_scale = ConstrainedParameter(initial=0.6, fixed=True, lower=0.1, upper=5)
A_variance = ConstrainedParameter(initial=0.01, fixed=True, lower=1e-5, upper=5)
λ0_variance = ConstrainedParameter(initial=0.01, fixed=True, lower=1e-5, upper=5)
...
offsets = Parameter(dims=(n_spaxels,), fixed=False, initial=0.003 * np.ones(n_spaxels))
